...
enum class AdaptiveQualityMode(val wire: String) {
    ...
    companion object {
        fun fromWire(wire: String?): AdaptiveQualityMode =
            when (wire) {
                Off.wire -> Off
...
enum class AdaptarrConnectionSaveResult {
    Saved,
    InvalidBaseUrl,
    InvalidToken,
    EncryptionFailed,
...
/**
 * Returns a normalized HTTP(S) Adaptarr endpoint, an empty string to clear it,
 * or null when the supplied value is unsafe or malformed.
 */
...
/**
 * Returns a normalized token, an empty string to clear it, or null when it
 * violates the token storage boundaries.
 */
internal fun normalizeAdaptarrToken(value: String): String? {
    ...
    if (codePoints !in ADAPTARR_TOKEN_MIN_CODE_POINTS..ADAPTARR_TOKEN_MAX_CODE_POINTS) return null
    ...
    if (utf8Bytes > ADAPTARR_TOKEN_MAX_UTF8_BYTES) return null
    return trimmed
}
